...
**The recording UI lives inside the `on_finish` callback**, not in a separate page. After the timeline completes, `showRecording()` overwrites `document.body` with a single Download JSON button and stashes the raw object on `window._sessionRecording` for console inspection. Note that `recording.end_reason` will be `null` here because we read the recording *during* `on_finish`, before jsPsych's internal `stop()` runs — this is expected, not a bug.
